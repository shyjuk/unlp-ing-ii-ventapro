...
public class BuscadorCliente extends Buscador<Cliente> {
	...
	protected ArrayList<Cliente> ejecutarBusqueda(AccesoDb db, Errores errores)
		...
		return Cliente.buscarCliente(db, this.getNombre(), this.getApellido(), this.getNroDocumento());
	}

	@Override
	protected void validarCriterios(Errores errores) {

		if (Utiles.esVacio(this.getNombre()) && Utiles.esVacio(this.getApellido()) && Utiles.esVacio(this.getNroDocumento())) {

			errores.setGeneral("Debe completar al menos uno de los criterios de busqueda.");
		} else {

			if (!Utiles.esVacio(this.getNroDocumento()))
				Validador.validarEntero(errores, "nroDocumento", this.getNroDocumento(), 0,
				99999999L, 8, false);
			...
			if (!Utiles.esVacio(this.getApellido()))
				Validador.validarLongitud(errores, "apellido", this.getApellido(),
						0, 20);			
